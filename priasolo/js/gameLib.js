...
//kotak rounded
	...
	function cekWarna(txt){
		...
		if (cl[0] == "hijautua") res = "#4CAF50";
if (cl[0] == "emas") res = "#D4AF37";
		var res2 = "none";
		if (cl.length>1){
			...
			if (cl[1] == "hijautua") res2 = "#4CAF50";
if (cl[1] == "emas") res2 = "#D4AF37";
		}
...
untuk membuat peta gunakan map editor tiling-editor
//------------------------------------------------------------*/
...
if(
    ...
    if(musuh.shootTimer == undefined){
        musuh.shootTimer = 0;
    }

    var jarakHero =
        Math.abs(
            game.charX - musuh.charX
        );

    if(jarakHero < 9){

        musuh.img =
            musuh.dataGambar.animShoot;

        if(game.charX > musuh.charX){
            musuh.skalaX = 1;
        }else{
            musuh.skalaX = -1;
        }
		...
		if (musuh.stat == 1){
			...
			if (musuh.sx>0){
				...
				if (cx < game.map.length){
					...
					if (game.map[cx][musuh.charY][0] == 1 ||
